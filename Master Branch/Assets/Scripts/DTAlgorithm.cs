//Borrowed the implemenation for the Delauny Algorithm from Nathis Green via GITHUB
// https://bitbucket.org/NathisGreen/pcgdungeons/overview
//if time in future, will implement own version
...
public class DTAlgorithm {
	...
	private void checkEdges(List<Edge> _list){
		...
		foreach(Triangle aTri in triangleList){
			if (aTri.checkContainsEdge(currentEdge)){
		...
		if (index == 2){
			...
			float angle0 = calculateVertexAngle(uniqueNodes[0].getVertex(), 
			                                    currentEdge.getNode0().getVertex(), 
			                                    currentEdge.getNode1().getVertex());
			
			float angle1 = calculateVertexAngle(uniqueNodes[1].getVertex(), 
			                                    currentEdge.getNode0().getVertex(), 
			                                    currentEdge.getNode1().getVertex());
